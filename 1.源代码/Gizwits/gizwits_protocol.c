...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagstepmotor)
    {
        bitFieldBitLen += stepmotor_LEN;
    }
    ...
    if(0x01 == aFlag->flagjrq)
    {
        bitFieldBitLen += jrq_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagstepmotor)
    {
        info->event[info->num] = EVENT_stepmotor;
        info->num++;
        dataPoints->valuestepmotor = gizVarlenDecompressionValue(bitFieldOffset,stepmotor_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += stepmotor_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagjrq)
    {
        info->event[info->num] = EVENT_jrq;
        info->num++;
        dataPoints->valuejrq = gizVarlenDecompressionValue(bitFieldOffset,jrq_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += jrq_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuestepmotor != cur->valuestepmotor)
    {
        GIZWITS_LOG("valuestepmotor Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagstepmotor = 1;
        ret = 1;
    }
    ...
    if(last->valuerain != cur->valuerain)
    {
        GIZWITS_LOG("valuerain Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagrain = 1;
        ret = 1;
    }
    if(last->valuejrq != cur->valuejrq)
    {
        GIZWITS_LOG("valuejrq Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagjrq = 1;
        ret = 1;
    }
    ...
    if(last->valueLux != cur->valueLux)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLux Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagLux = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagstepmotor)
    {
        gizVarlenCompressValue(bitFieldOffset,stepmotor_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuestepmotor);
        bitFieldOffset += stepmotor_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagrain)
    {
        gizVarlenCompressValue(bitFieldOffset,rain_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuerain);
        bitFieldOffset += rain_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagjrq)
    {
        gizVarlenCompressValue(bitFieldOffset,jrq_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuejrq);
        bitFieldOffset += jrq_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagLux)
    {
        devStatusTmp.valueLux = exchangeBytes(gizY2X(Lux_RATIO,  Lux_ADDITION, dataPoints->valueLux)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueLux,Lux_LEN);
        byteFieldOffset += Lux_LEN;
    }
